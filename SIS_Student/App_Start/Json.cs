...
namespace _API
{
    public class Json
    {
        ...
        public static NameValueCollection BuildNVCFromJson(string json)
        {
            ...
            if (!json.Contains("The request was aborted: Could not create SSL/TLS secure channel"))
            {
                JavaScriptSerializer deserializer = new JavaScriptSerializer();
                dict = deserializer.Deserialize<Dictionary<string, object>>(json);

            }
            else
            {
                dict.Add("Could not create SSL/TLS secure channel", new object());
            }
