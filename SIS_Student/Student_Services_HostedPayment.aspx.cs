...
namespace SIS_Student
{
    public partial class Student_Services_HostedPayment : System.Web.UI.Page
    {
        ...
        public void SessionCreate(string orderid, double amount, string desc)
        {
            using (var httpClient = new HttpClient())
            {
                using (var request = new HttpRequestMessage(new HttpMethod("POST"), "https://eu-gateway.mastercard.com/api/nvp/version/56"))
                {
                    ...
                    //contentList.Add("apiPassword=deeb63937d9daf76831ef22114223968");//Test
                    contentList.Add("apiPassword=f2474867cc7dbba096b0e4373271d3f0");//Live
                    contentList.Add("interaction.operation=PURCHASE");
                    //contentList.Add("interaction.returnUrl=https://localhost:44387/Result");
                    //contentList.Add("apiUsername=merchant.TEST7006448");//Test
                    contentList.Add("apiUsername=merchant.7006448");//Live
                    //contentList.Add("merchant=TEST7006448");//Test
                    contentList.Add("merchant=7006448");//Live
        ...
        protected void chkAgree_CheckedChanged(object sender, EventArgs e)
        {
            ...
            insert_online_payment(orderid, hdnACC.Value, hdnSID.Value, Session["CurrentService"].ToString(), hdnAmount.Value);
